...
// Entity: 			xw.c
// Version:			1.0
// Description: 	This file is used to write an output
//
// Additional Comments:
//
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
void write_addr(int addr, int value)
{
	if (_lseek(fdw_command_CR, addr, SEEK_SET) < 0) {
...
void xillybus_init()
{
	...
	*/
	...
	fdw_command_CR = _open("\\\\.\\xillybus_mem_32", O_WRONLY | _O_BINARY);

	if (fdw_command_CR < 0) {
		if (errno == ENODEV)
			fprintf(stderr, "(Maybe fdw_command_CR a read-only file?)\n");
			perror("Failed to open devfile fdw_command_CR");
			exit(1);
	}

	fdw = _open("\\\\.\\xillybus_write_32", O_WRONLY | _O_BINARY);
